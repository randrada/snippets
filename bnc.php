<?php
if(empty($estaentradafecha)) { header("Location: index.php"); exit; }

if (isset($_GET['isbnleido'])) {
if($_GET['nuevselec'] == "true") { $isbn_ob = $_GET['isbnleido']; } else {
...
if(empty($coi[1]) == false) { $auty = $coi[1]; }
$tituloar = trim(str_replace(array('/', ' :'), array('', ''), entre($contenido, 'width="40%" valign=top>', '</td>')));
$fechaar = trim(entre($contenido, 'width="5%" valign=top>', '</td>'));
$entrada = trim(entre($contenido, 'set_entry=', '&format='));
if ($auty <> '' && $auty <> "<BR>") { $auty = "<span class=\"versalitas\">" . $auty . "</span>: "; } else { $auty = "&nbsp;"; }
...
if(empty($coi[1]) == false) { $isbn = $coi[1]; }
break;
case "100":
...
if(empty($coi[1]) == false) { $autor .= " " . $coi[1]; }
...
case "260":
preg_match('@^\|a (?:\[?)(.*?)(?:\]?)(?: ;)?(?:,)?(?: \:| \|)@', $significado, $coi);
if(empty($coi[1]) == false) { $lugar = $coi[1]; }
preg_match_all('@\|b (?:\[)?(.*?)(?:\])?(?: \:| \||$)@', $significado, $coi);
foreach($coi[1] as $variab) {
if(empty($variab) == false) {
if(empty($editorial) == false) { $editorial .= "; "; }
@$editorial .= $variab;
}
}
if(substr(@$editorial, strlen(@$editorial)-1, 1) == ",") { $editorial = substr(@$editorial, 0, strlen(@$editorial)-1); }
if(substr(@$editorial, strlen(@$editorial)-1, 1) == "]") { $editorial = substr(@$editorial, 0, strlen(@$editorial)-1); }
preg_match('@^(?:.*?)\|c (?:\[?)(?:c)?([0-9]{4})(?:\]?)(?:.)?@', $significado, $coi);
if(empty($coi[1]) == false) { $fecha = $coi[1]; }
break;
case "300":
preg_match('@^\|a ([0-9]{1,6})(?: p\.)(?:.*?)(?: \|)@', $significado, $coi);
if(empty($coi[1]) == false) { $paginas = $coi[1]; }
preg_match('@^(?:.*?)\|c ([0-9]*)(?: cm.)?@', $significado, $coi);
if(empty($coi[1]) == false) { $altura = $coi[1]; }
break;
case "245":
preg_match('@^\|a (.*?)(?: \:)?(?: \|| \/)@', $significado, $coi);
if(empty($coi[1]) == false) {
...
preg_match('@\|b (.*?)(?: \:)?(?: \|| \/)@', $significado, $coi);
if(empty($coi[1]) == false) { $titulo .= ": " . $coi[1]; }
}
break;
case "440":
preg_match('@^\|a (.*?)(?:;| \||$)@', $significado, $coi);
...
if(empty($coi[1]) == false) { $numero = rastrearcifras($coi[1]); }
}
break;
case "250":
preg_match('@^\|a (.*)(?: \||$)@', $significado, $coi);
if(empty($coi[1]) == false) { $edicion = $coi[1]; }
break;
case "500":
preg_match('@^\|a (.*)(?: \|)?@', $significado, $coi);
if(empty($coi[1]) == false) { if(empty($nota) == false) { $nota .= " "; } @$nota .= $coi[1]; }
break;
case "504":
preg_match('@^\|a (.*)(?: \|)?@', $significado, $coi);
if(empty($coi[1]) == false) { if(empty($nota) == false) { $nota .= " "; } @$nota .= $coi[1]; }
...
case "650":
preg_match('@^\|a (.*?)(?: \||$)@', $significado, $coi);
if(empty($coi[1]) == false && $coi[1] <> @$materias) { if(empty($materias) == false) { $materias .= "|OC|"; } @$materias .= $coi[1]; }
break;
case "700":
preg_match('@^\|a (.*?)(?:(,|.)? \||$)@', $significado, $coi);
if(empty($coi[1]) == false) { if(empty($editor) == false) { $editor .= "; "; } @$editor .= $coi[1]; }
break;
case "240":
preg_match('@^\|a (?:\[)?(.*?)(?:\])?(?:(.)? \||$)@', $significado, $coi);
if(empty($coi[1]) == false) {
...
if(empty($coi[1]) == false) { $idioma = $coi[1]; }
}
break;
} } }
